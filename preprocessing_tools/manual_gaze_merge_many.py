...
def main():
    ...
    parser.add_argument("files", nargs="+", help="gaze files from Solomon Coder")
    parser.add_argument("--outdir", nargs=1, required=True, help="output directory")
    ...
    for gaze_file in filelist:
        dat_file = re.sub(r"^[A-Za-z]+_", "", gaze_file)
        dat_file = dat_file.replace(".csv", ".dat")
        #print("Augmenting {} with {} to make {}".format(dat_file, gaze_file, outpath+dat_file))
        augmented_dat = merge_dat(dat_file, gaze_file)
        augmented_dat.to_csv(os.path.join(outpath,dat_file), sep=" ", na_rep='.', index=False, columns=["VidTime", "SimTime", "LonAccel",
                    "LatAccel", "Throttle",
                    "Brake", "HeadwayTime", "RoadOffset", "XPos", "YPos", "ZPos", "TaskID",
                    "TaskStatus", "TaskFail", "DriveID", "ParticipantID", "gaze", "gazenum", "taskblocks"])
