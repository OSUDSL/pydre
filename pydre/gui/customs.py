    ...
    def combo_box(cb, items, val=None, style=None):
        ...
        if style is not None:
            ...
            widget.setStyleSheet(
                str(files("pydre.gui.stylesheets").joinpath("widget.css"))
            )
...
class FiltersTree(QTreeWidget):
    ...
    def update(self, idx, key, val):
        ...
        if key == "function":
            ...
            def cb(e):
                return self.handle_func_update(idx, val, e)
...
class ProjectTree(QTreeWidget):
    ...
    def add_roi(self, item=None, idx=None, expanded=None):
        ...
        item = (
            item
            if item is not None
            else {"type": f'new_roi_{self.counters["rois"]}', "filename": f"<roi_file>"}
        )
