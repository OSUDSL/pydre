...
class TimeROI():
	...
	def split(self, datalist):
		...
		for row in range(1, len(table)):
			...
			for roi in table[row]:
				...
				drives = []
				source_files = []
				for drive_info in getCellInfo(table[row][titles[roi]]):
	...
	def getCellInfo(cell_content):
		...
		cell_regex = "(\d?:?\d?\d:\d\d)-(\d?:?\d?\d:\d\d)#?(\d+)?" #GROUPS 0 = start time, 1 = end time, 2 = driveID (1 if not specified)
		time_regex = "(?:(\d):)?(\d\d|^\d):(\d\d)" #GROUPS: 0 = hour digit (if nec), 1 = minute digit, 2 = seconds digit
		cell_info = []
		for drive in cell_content.split():
			drive_info = re.match(cell_regex, drive)
			
			driveID = 1
			if (drive_info.groups()[2] != None):
