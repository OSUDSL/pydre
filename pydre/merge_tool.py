...
class MergeTool:
    ...
    def sequential_merge(self, input_directory, exp_index):
        ...
        file_list = sorted(
            glob.glob(input_directory + "/" + regular_expressions_glob[exp_index])
        )
        ...
        for key in subject_groups:
            ...
            out_frame["SourceId"] = source_dir[
                max(source_dir.find("\\"), source_dir.find("/")) :
            ]
            for drive in drive_group[1:]:
                ...
                source_dir = drive
                next_frame["SourceId"] = source_dir[
                    max(source_dir.find("\\"), source_dir.find("/")) :
                ]
                out_frame = out_frame.append(next_frame, ignore_index=True)
            out_frame.to_csv(
                os.path.join(out_dir_name, out_name), sep=" ", na_rep=".", index=False
            )
    ...
    def spatial_merge(self, input_directory, exp_index):
        ...
        out_dir_name = os.path.join(input_directory, "MergedData")
        file_list = glob.glob(
            input_directory + "/" + regular_expressions_glob[exp_index]
        )
    ...
    def __init__(
        self, input_directory, merge_type="spatial", regular_expression_index=0
    ):
        global regular_expressions
