...
def findFirstTimeAboveVel(
    drivedata: tests.sample_pydre.core.DriveData, cutoff: float = 25
):
    timestepID = -1
    breakOut = False
...
def lanePosition(
    ...
    for d in drivedata.data:
        df = pandas.DataFrame(
            d, columns=("SimTime", "Lane", "LaneOffset")
        )  # drop other columns
        LPout = None
        if df.size > 0:
            if laneInfo in ["mean", "Mean"]:
                ...
                shifts = (
                    compare.loc[compare == True] == is_violating.loc[compare == True]
                )
                ...
                if violations.size > 0:
                    ...
                    LPout = sum(
                        deltas.SimTime[(deltas.SimTime < 0.5) & (deltas.SimTime > 0)]
                    )
            else:
                print(
...
def tailgatingTime(drivedata: tests.sample_pydre.core.DriveData, cutoff=2):
    ...
    for d in drivedata.data:
        ...
        tail_data = table[
            (table.HeadwayTime > 0)
            & (table.HeadwayTime < cutoff)
            & (abs(table.delta_t) < 0.5)
        ]
        tail_time += tail_data["delta_t"][abs(table.delta_t) < 0.5].sum()
    return tail_time


def tailgatingPercentage(
    drivedata: tests.sample_pydre.core.DriveData, cutoff: float = 2
):
    total_time = 0
    tail_time = 0
    for d in drivedata.data:
        ...
        tail_time += tail_data["delta_t"][abs(table.delta_t) < 0.5].sum()
        total_time += table["delta_t"][abs(table.delta_t) < 0.5].sum()
    return tail_time / total_time


def boxMetrics(
    drivedata: tests.sample_pydre.core.DriveData, cutoff: float = 0, stat: str = "count"
):
    ...
    for d in drivedata.data:
        df = pandas.DataFrame(
            d, columns=("SimTime", "FeedbackButton", "BoxAppears")
        )  # drop other columns
        df = pandas.DataFrame.drop_duplicates(
            df.dropna(axis=0, how="any")
        )  # remove nans and drop duplicates
        ...
        for counter in range(0, len(indicesBoxOn)):
            ...
            buttonClickedIndices = buttonClickeddf[buttonClickeddf.values > 0.5].index[
                0:
            ]
...
def tbiReaction(
    drivedata: tests.sample_pydre.core.DriveData, type: str = "brake", index: int = 0
):
    for d in drivedata.data:
        df = pandas.DataFrame(
            d,
...
def ecoCar(
    ...
    for d in drivedata.data:
        ...
        indicesToggleOn = toggleOndf[toggleOndf.values > 0.5].index[0:]
        ...
        for counter in range(0, len(indicesToggleOn)):
            ...
            warningPlus15 = df[
                (df.SimTime >= startWarning) & (df.SimTime <= startWarning + 15)
            ]
            ...
            if failureCodedf.loc[warningOn] == int(FailCode):
                ...
                throttleVector = throttleDuringWarning[
                    throttleDuringWarning >= (initial_throttle + thresholdThrottle)
                ]
                ...
                autonomousToggleOn = autonomousOndf[autonomousOndf.values > 0.5].index[
                    0:
                ]
...
def gazeNHTSA(drivedata: tests.sample_pydre.core.DriveData):
    ...
    for d in drivedata.data:
        df = pandas.DataFrame(
            d, columns=("VidTime", "gaze", "gazenum", "TaskFail")
        )  # drop other columns
        df = pandas.DataFrame.drop_duplicates(
            df.dropna(axis=0, how="any")
        )  # remove nans and drop duplicates
...
def speedbumpHondaGaze(drivedata: tests.sample_pydre.core.DriveData):
    ...
    for d in drivedata.data:
        df = pandas.DataFrame(
            d, columns=("DatTime", "gaze", "gazenum", "TaskNum")
        )  # drop other columns
        df = pandas.DataFrame.drop_duplicates(
            df.dropna(axis=0, how="any")
        )  # remove nans and drop duplicates
