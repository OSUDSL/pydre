...
def sliceByTime(begin: float, end: float, column: str, drive_data: pandas.DataFrame):
    ...
    try:
        dataframeslice = drive_data[
            (drive_data[column] >= begin) & (drive_data[column] <= end)
        ]
    except KeyError:
        logger.error(
        ...
        if len(drivedata.data) > 1:
            logger.warning(
                "Subject {} roi {} contains  more than one DataFrame. Only the first element will be in the merged output.".format(
                    drivedata.SubjectID, DriveData.roi
                )
            )
        ...
        if len(tomerge) > 1:
            ...
            while i < len(tomerge) - 1:
                ...
                if tomerge[i].SubjectID != subject:
                    logger.warning(
                        "Merging data for multiple subjects {} and {}. Only the first will be used".format(
                            subject, tomerge[i].SubjectID
                        )
                    )
                driveIDs.append(
                    list(tomerge[i].DriveID)
                )  # Some may be added twice. Is this desireable?
                if tomerge[i].roi != roi:
                    logger.warning(
                        "Merging data for multiple rois {} and {}. Only the first will be used.".format(
                            roi, tomerge[i].roi
                        )
                    )
