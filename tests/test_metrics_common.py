...
def test_stoppingDist():
    df = pl.DataFrame({
        "Velocity": [10.0, 5.0, 0.005, 0.0, 0.0],
        "XPos": [0, 10, 20, 25, 30]
    })
...
def test_reactionBrakeFirstTrue():
    ...
    df_none = pl.DataFrame({
        "SimTime": [0.0, 1.0, 2.0],
        "BrakeSignal": [1.0, 2.0, 3.0]
    })
    dd_none = pydre.core.DriveData.init_test(df_none, "test_reactionBrakeFirstTrue_none.dat")
    result_none = pydre.metrics.common.reactionBrakeFirstTrue(dd_none, var="BrakeSignal")
    assert result_none is None
...
def test_reactionTimeEventTrue():
    ...
    result = pydre.metrics.common.reactionTimeEventTrue(dd, var1="BrakeSignal", var2="Steering")
    assert result == 2.0
    ...
    result2 = pydre.metrics.common.reactionTimeEventTrue(dd2, var1="BrakeSignal", var2="Steering")
    ...
    result3 = pydre.metrics.common.reactionTimeEventTrue(dd3, var1="BrakeSignal", var2="Steering")
...
def test_closeFollowing():
    ...
    # → delta_t = 0.1 + 0.1 = 0.2
    assert pytest.approx(pydre.metrics.common.closeFollowing(dd1, threshold=2), 0.01) == 0.2


def test_steeringReversalRate():
    simtime = [i * 0.05 for i in range(20)]
    steer = [0.0, 0.3, 0.0, -0.3, 0.0, 0.3, 0.0, -0.3, 0.0, 0.3,
...
def test_leadVehicleCollision():
    ...
    df_none = pl.DataFrame({
        "SimTime": [0.0, 0.1, 0.2],
        "HeadwayDistance": [3.0, 3.1, 3.2]
    })
    dd_none = pydre.core.DriveData.init_test(df_none, "test_leadVehicleCollision_none.dat")
...
def test_laneViolations():
    df = pl.DataFrame({
        "LaneOffset": [0.0, 1.5, 2.0, 1.0, 2.5, 1.0],
        "Lane": [2, 2, 2, 2, 2, 2]
    })
...
def test_timeToOutsideThreshold():
    df = pl.DataFrame({
        "SimTime": [0.0, 1.0, 2.0, 3.0],
        "Steer": [0.0, 0.1, 0.2, 0.6]
    })
    dd = pydre.core.DriveData.init_test(df, "test_timeToOutsideThreshold.dat")
    result = pydre.metrics.common.timeToOutsideThreshold(dd, var="Steer", threshold_high=0.5)
    assert result == 3.0
    ...
    result2 = pydre.metrics.common.timeToOutsideThreshold(dd2, var="Steer", threshold_high=0.5)
